...
// (Opcional) Se quiser parametrizar as frequências por #define:
#define ONDA_FREQUENCIA_HZ_17   1.0f
#define ONDA_FREQUENCIA_HZ_28   5.0f
...
int main(void) {
    ...
    tarefa_onda_quadrada_cfg_t cfg = {
        ...
        .gpio_led_g         = 11,
        .gpio_led_b         = 12,
        .led_active_low     = false,     // true se seu RGB for comum-ânodo
    ...
    BaseType_t ok = tarefa_onda_quadrada_start(&cfg, NULL);
    configASSERT(ok == pdPASS);

    tarefa_onda_quadrada_cfg_t cfh = {
        .gpio_wave          = 17,
        .frequencia         = 5.0f,
        .gpio_led_r         = 13,
        .gpio_led_g         = 11,
        ...
        .task_stack         = 1024,
        .core_affinity_mask = (1u << 0)  // core 1; use 0 para sem afinidade
    };

    BaseType_t oi = tarefa_onda_quadrada_start(&cfh, NULL);
    configASSERT(oi == pdPASS);
   // ---- RTOS ----
    vTaskStartScheduler();
