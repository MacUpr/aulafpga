...
static void tarefa_botao_b_entry(void *param) {
    (void)param;

    cfg_in_pullup(GPIO_BOTAO_B);
    cfg_out_zero(PIN_OUT_BTN_B);

    bool stable_pressed = (gpio_get(GPIO_BOTAO_B) == 0);
    ...
    for (;;) {
        bool raw_pressed = (gpio_get(GPIO_BOTAO_B) == 0);

        if (raw_pressed != stable_pressed) {
            TickType_t now = xTaskGetTickCount();
            if ((now - t_mark) >= tdeb) {
#if BTN_B_PULSE_MODE
                gpio_put(PIN_OUT_BTN_B, raw_pressed ? 1 : 0);
#else
                if (raw_pressed && !stable_pressed) {
                    gpio_put(PIN_OUT_BTN_B, !gpio_get_out_level(PIN_OUT_BTN_B));
                }
#endif
...
void criar_tarefa_botao_b(UBaseType_t prio, UBaseType_t core_mask) {
    ...
    BaseType_t ok = xTaskCreate(tarefa_botao_b_entry, "btnB", 768, NULL, prio, &th);
