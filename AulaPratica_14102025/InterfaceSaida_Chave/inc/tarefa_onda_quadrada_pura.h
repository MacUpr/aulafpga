#ifndef TAREFA_ONDA_QUADRADA_PURA_H
#define TAREFA_ONDA_QUADRADA_PURA_H
...
#define ONDA_FREQUENCIA_HZ   5   // Ex.: 2 Hz -> período total = 0,5 s
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
} tarefa_onda_quadrada_cfg_p;

BaseType_t tarefa_onda_quadrada_start2(const tarefa_onda_quadrada_cfg_p *cfg, TaskHandle_t *handle_out);
...
#endif // TAREFA_ONDA_QUADRADA_PURA_H
