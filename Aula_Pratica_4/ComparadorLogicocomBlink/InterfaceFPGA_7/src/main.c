...
// >>> NOVO: LED verde fixo ao iniciar
...
// ==== Prioridades ====
// Maior -> menor: WORD6 > JOY > A > B >= OLED ~ LED
...
// >>> NOVO: tarefa que apenas seta o LED e se suspende (pode ser baixa)
...
int main(void) {
    ...
    printf("=== INICIANDO SISTEMA: WORD6 + OLED + BOTOES ===\n");
    ...
    // >>> NOVO: fixa LED RGB em VERDE ao iniciar (suspende-se em seguida)
    // Usa os pinos do LED_RGB_A por padrão (R=13, G=11, B=12) e ativo-baixo.
