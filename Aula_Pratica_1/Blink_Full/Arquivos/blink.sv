// ====================================================================
// top.sv  — Blink parametrizável (ON/OFF em ms) + saída de sincronismo
...
module top #(
    ...
    output logic led,        // saída do "blink" (pode ir para um pino ou LED on-board)
    output logic sync_out    // saída sincronizada (mesma fase do blink)
);

    // ----------------------------------------------------------------
    // Conversão ms -> ticks
    // (Para os valores usuais — 25 MHz * 500 ms / 1000 = 12_500_000 — 
    //  cabem em 32 bits sem problemas.)
    // ----------------------------------------------------------------
    ...
    // ----------------------------------------------------------------
    // Saídas — contínuas (combinacionais) para evitar atraso de 1 ciclo
    // ----------------------------------------------------------------
    assign led      = blink_on;
    assign sync_out = blink_on;
