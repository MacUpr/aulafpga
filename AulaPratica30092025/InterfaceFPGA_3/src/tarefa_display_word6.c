...
static void desenhar_cabecalho_toggle(void)
{
    ...
    ssd1306_draw_utf8_multiline(buf, 0, 0, "BOTÕES A/B/JOY: PULSE", w_pix, h_pix);
}
...
static void desenhar_cabecalhos_saidas(void)
{
    ...
    ssd1306_draw_utf8_multiline(buf, 0, 1*8, "Out_D:", w_pix, h_pix);
    ssd1306_draw_utf8_multiline(buf, 60,1*8, bit_txt(outA), w_pix, h_pix);

    ssd1306_draw_utf8_multiline(buf, 0, 2*8, "Out_E:", w_pix, h_pix);
    ssd1306_draw_utf8_multiline(buf, 60,2*8, bit_txt(outB), w_pix, h_pix);

    ssd1306_draw_utf8_multiline(buf, 0, 3*8, "Out_F:", w_pix, h_pix);
    ssd1306_draw_utf8_multiline(buf, 60,3*8, bit_txt(outJ), w_pix, h_pix);

    // Rodapé (opcional) na página 4
    ssd1306_draw_utf8_multiline(buf, 0, 4*8, "BitDogLab_PULSE", w_pix, h_pix);
}
